...
/////////////////////////////////////////////////////////////////////// sliders
var strokeValue;
var strokeSlider = document.getElementById('slider1');
strokeSlider.value = 50;
...
strokeSlider.addEventListener('change', function () {
    strokeValue = mapStroke(this.value);
});
...
function mapStroke(val) {
    return map(val, 1, 100, 5, 15);
}

function mapSize(val) {
    return map(val, 1, 100, 1, 1.7);
}

function mapColor(val) {
    return map(val, 1, 100, 0, 1);
}

function mapSpeed(val) {
    return map(val, 1, 100, 0.5, 1);
}
...
function playNote(e) {
    ...
    if (!key.classList.contains('fired')) {
        ...
        // custom
        speed += (0.00008 * notePressed)*speedValue;
        strokeAlpha += 1;
...
function draw() {
    if (active) {
        ...
        speed = constrain(lerp(speed, 0, 0.03),0,0.009);
        strokeAlpha = constrain(lerp(strokeAlpha, 0, 0.025),0,10);
        offset += speed;
        ...
        for (var i = 0; i < x.length; i++) {
            if (color) {
                stroke(255 - (rangeR + sin(frameCount * r) * ((255 - rangeR) / 2) * colorValue),
                    255 - (rangeG + sin(frameCount * g) * ((255 - rangeG) / 2) * colorValue),
                    255 - (rangeB + sin(frameCount * b) * ((255 - rangeB) / 2) * colorValue), strokeAlpha);
            }
